...
var seek2 = function(objData, cutData, elems, parsed, lvl, prevs, callback)
{
    ...
    if(type === 'delta1')
    {
        dellen = pf.getBaseObjectOffset(cutData.slice(cut, cutData.length), objData.length - cutData.length);
        cut += dellen[0];
    }
    else if(type === 'delta2')
    {
    ...
    else
    {
        ...
        else
        {
            if(type !== 'delta1')
                ...
                prevs.push(new objectInPack(last, type, objData.length - cutData.length - cut, 
                    objData.length - cutData.length , deflatedData.length, inflatedLength, dellen[1]));
            seek2(objData, cutData.slice(deflatedData.length, cutData.length),
            elems, parsed+1, -1, prevs, callback);
        }
    }
