 ...
 */
function PackIndex(fanout, objectsCount, objects, crc32, offset32, offset64, packSha1, sha1)
{
	this.fanout = fanout;
...
function parsePackIndex(buffer, callback)
{
	...
	for (var i = 0; i < objectsCount; ++i)
	{
		...
		crc32[i] = buffer.readUInt32BE(index);
		index += 4;
	}
	...
	var packSha1 = buffer.toString('hex', buffer.length-40, buffer.length-20);
	var packIndexSha1 = buffer.toString('hex', buffer.length-20, buffer.length);
	var packIdx = new PackIndex(fanout, objectsCount, objects, crc32, offset32, offset64, packSha1, packIndexSha1);
...
PackIndex.prototype.getOffset = function (index)
{
	if (this.offset32[index] & IS_O64)
	{
		...
		var offset = Math.pow(2,32)*firstByte + secondByte;
	}
	else 
	{	
