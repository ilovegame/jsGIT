...
Refs.prototype.getBranchesNames = function (callback) {
    var refsObject = this;
    var path = ph.join(this.path, '/refs/heads/');
...
Refs.prototype.getSHA_1OfTheActiveBranch = function(callback) {
    ...
    gitutils.getHeadSha1(this.path.slice(0, this.path.length - 4) + ".git", function(err, headSHA_1) {
        if (err) {
            callback(err, '');
            return;
        }
...
Refs.prototype.getBranchNameUsingSHA_1 = function(SHA_1, callback) {
    ...
    fs.readdir(path, function (err, branchesNames) {
        if (err) {
            callback(err, '');
            return;
        }
        var result = '';
        var searchName = function(names, index, callback) {
            if (names.length == index) {
                callback('')
                return
            }
            var path = ph.join(refsObject.path, '/refs/heads/',names[index]);
            fs.readFile(path,
                ...
                searchName(names, index + 1, function(err) {
                    if (err) {
                        callback(err);
                        return;
                    }
        ...
        searchName(branchesNames, 0, function(err) {
            if (err) {
                callback(err, '');
                return;
            }
        ...
        for (index in branchesNames) {
            var path = ph.join(refsObject.path, '/refs/heads/',branchesNames[index]);
            fs.readFile(path,
                'utf8', function(err, content) {
                if (err) {
                    callback(err, '');
...
Refs.prototype.getActiveBranchName = function(callback) {
    ...
    fs.readFile(refsObject.path + '/HEAD', 'utf8', function(err, HEADcontent) {
        if (err) {
            callback(err, '');
            return;
        }

        if (HEADcontent.split(' ')[0] == 'ref:') {
            
            var parts = HEADcontent.split('\n')[0].split('/');
...
Refs.prototype.getTagsNames = function (callback) {
    ...
    fs.readdir(this.path + '/refs/tags/', function (err, files) {
        ...
        var data = new Array();
        var createData = function(names, index, callback) {
            if (names.length == index) {
                callback('');
                return;
            }
            fs.readFile(refsObject.path + '/refs/tags/' + names[index], 'utf8', function(err, tagSHA_1) {
                if (err) {
                    callback(err, '');
                    return;
                }
                tagSHA_1 = tagSHA_1.split('\n')[0];
                tag.readFileToTag(tagSHA_1, refsObject.path, function(err, tagObject) {
                    ...
                    tagInfo['name'] = names[index];
                    tagInfo['tag_SHA_1'] = tagSHA_1;
                    tagInfo['commit_SHA_1'] = tagObject['objectId'];
                    data.push(tagInfo);
                    createData(names, index + 1, function(err) {
                        if (err) {
                            callback(err);
                            return;
                        }
        ...
        createData(files, 0, function(err) {
            if (err) {
                callback(err, '');
                return
            }
            refsObject.packedRefsManager.getTagsNames(function(tags) {
                var tags = data.concat(tags);
...
/**
 * Gets all remotes names
 * @param callback callback(err, names)
 */
Refs.prototype.getRemotesNames = function(callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes', function(exists) {
        if (!exists) {
            callback('', new Array());
            return;
        }
        fs.readdir(refsObject.path + '/refs/remotes', function(err, filesNames) {
            if (err) {
                callback(err, '');
                return;
            }
...
/**
 * Gets branch details using remote and branch names
 * @param remote remote name
 * @param branch branch name
 * @param callback callback(err, details)
 */
Refs.prototype.getBranchDetails = function(remote, branch, callback) {
    var refsObject = this;

    fs.exists(refsObject.path + '/refs/remotes/' + remote, function(exists) {
        if (exists) {
            fs.exists(refsObject.path + '/refs/remotes/' + remote + '/' + branch, function(exists) {
                if (exists) {
                    fs.readFile(refsObject.path + '/refs/remotes/' + remote + '/' + branch, 'utf8', function(err, branchSHA_1) {
                        if (err) {
                            callback(err, '');
                            return;
                        }
                        ...
                        details['name'] = branch;
                        details['SHA_1'] = branchSHA_1.split('\n')[0];
                        callback('', details);
                        return;
                    });
                } else {
                    refsObject.packedRefsManager.getBranchSHA_1UsingName('refs/remotes/' + remote + '/' + branch, function(result) {
                        if (result == '') {
                            callback('error: pathspec \'' + remote + '/' + branch + '\' did not match any file(s) known to git', '');
                            return;
                        }
                        var details = new Array();
                        details['remote'] = remote;
                        details['name'] = branch;
                        details['SHA_1'] = result;
                        callback('', details);
                        return;
                    });
                }
...
/**
 * Creates empty directory in .git/refs/remotes for new remote
 * @param remoteName name for new remote
 * @param callback callback(err)
 */
Refs.prototype.addRemote = function(remoteName, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes', function(exists) {
        if (exists) {
            fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
                if (exists) {
                    callback('fatal: remote ' + remoteName + ' already exists');
                    return;
                }
                fs.mkdir(refsObject.path + '/refs/remotes/' + remoteName, 0777, function(err) {
                    if (err) {
                        callback(err);
                        return;
                    }
            ...
            fs.mkdir(refsObject.path + '/refs/remotes', 0777, function(err) {
                if (err) {
                    callback(err);
                    return;
                }
...
/**
 * Removes remote directory from .git/refs/remotes
 * @param remoteName name of the remote
 * @param callback callback(err)
 */
Refs.prototype.removeRemote = function(remoteName, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
        if (!exists) {
            callback('error: remote ' + remoteName + ' does not exist');
            return;
        }
        var removeDirectoryContent = function(path, objects, index, callback) {
            if (objects.length == index) {
                callback('');
                return;
            }
            fs.unlink(path + '/' + objects[index], function(err) {
                if (err) {
                    callback(err);
                    return;
                }
                removeDirectoryContent(path, objects, index + 1, function(err) {
                    if (err) {
                        callback(err);
                        return;
                    }
        ...
        fs.readdir(refsObject.path + '/refs/remotes/' + remoteName, function(err, objects) {
            if (err) {
                callback(err);
                return;
            }
            removeDirectoryContent(refsObject.path + '/refs/remotes/' + remoteName, objects, 0, function(err) {
                if (err) {
                    callback(err);
                    return;
                }
                fs.rmdir(refsObject.path + '/refs/remotes/' + remoteName, function(err) {
                    if (err) {
                        callback(err);
                        return;
                    }
...
/**
 * Creates file with branch name in selected remote containing SHA_1
 * @param remoteName name of remote
 * @param branchName name of branch
 * @param content content for branch
 * @param callback callback(err)
 */
Refs.prototype.setBranchInRemote = function(remoteName, branchName, content, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
        if (!exists) {
            callback('error: remote ' + remoteName + ' does not exist');
        ...
        fs.writeFile(refsObject.path + '/refs/remotes/' + remoteName + '/' + branchName, content, function(err) {
            if (err) {
                callback('');
                return
            }
...
/**
 * Creates file HEAD in selected remote
 * @param remoteName name of remote
 * @param content content of HEAD file
 * @param callback callback(err)
 */
 Refs.prototype.setRemoteHEAD = function(remoteName, content, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
        if (!exists) {
            callback('error: remote ' + remoteName + ' does not exist');
        ...
        fs.writeFile(refsObject.path + '/refs/remotes/' + remoteName + '/HEAD', content, function(err) {
            if (err) {
                callback('');
                return
            }
...
/**
 * Removes file representing a branch from .git/refs/remotes/remoteName
 * @param remoteName name of a remote
 * @param branchName name of a branch
 * @param callback callback(err)
 */
Refs.prototype.removeBranchFromRemote = function(remoteName, branchName, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
        if(!exists) {
            callback('error: remote ' + remoteName + ' does not exist');
        ...
        fs.exists(refsObject.path + '/refs/remotes/' + remoteName + '/' + branchName, function(exists) {
            if (!exists) {
                callback('error: branch ' + branchName + ' does not exist');
                return;
            }
            fs.unlink(refsObject.path + '/refs/remotes/' + remoteName + '/' + branchName, function(err) {
                if (err) {
                    callback(err);
                    return;
                }
...
/**
 * Gets branches names which come from selected remote
 * @param remoteName name of the remote
 * @param callback callback(err, remoteBranchesNames)
 */
Refs.prototype.getRemoteBranchesNames = function(remoteName, callback) {
    var refsObject = this;
    fs.exists(refsObject.path + '/refs/remotes/' + remoteName, function(exists) {
        if(!exists) {
            callback('error: remote ' + remoteName + ' does not exist', '');
        ...
        fs.readdir(refsObject.path + '/refs/remotes/' + remoteName, function(err, objects) {
            ...
            var remoteBranches = new Array();
            for (var branchIndex in objects) {
                if (objects[branchIndex] == 'HEAD') {
                    continue;
                }
                remoteBranches.push(objects[branchIndex]);
            }
            refsObject.packedRefsManager.getRemoteBranchesName(remoteName, function(packedBranchesNames) {
                remoteBranches = remoteBranches.concat(packedBranchesNames);
                callback('', remoteBranches);
            });
        });
    });
}
