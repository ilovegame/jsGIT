...
exports.gitStatus = function(repopath, callback) {
                    ...
                    if(workdir[tmparray[0]] !== undefined) {
                        if(index.objects[tmparray[0]]['mtime'] !== workdir[tmparray[0]]['mtime']) {
                            ...
                            var tmpfunction = function(filename) {
                                fs.readFile(ph.join(ph.normalize(ph.join(repopath,'..')), filename), function(err, data) {
                                    if(err) callback(err);
                                    else {
                                        bM.fileToBlob(data, function(err, data) {                                            
                                            var sha1 = utils.getSha1(data);
                                            if (sha1  !== 
                                                utils.convertDigestToHex(new Buffer(index.objects[filename]['SHA_1'], 'hex')) ) {
                                                // in index, in workdir, content differs => modified
                                                if(result.conflicts.indexOf(filename) === -1)
    ...
    //read index
    var index = new gitindex.Index(repopath, function(err) {
        if (err) {
            callback(err);
            return;
        }
