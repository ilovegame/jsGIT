...
function getHeadSha1(repopath, callback) {
    fs.readFile(ph.join(repopath,'HEAD'), function (err, data) {
        if (err) {
            callback(err);    
            return;
        }
        ...
        if (datastr.substr(0,5) === refs) {
            fs.readFile(ph.join(repopath, datastr.substr(5, datastr.length - 5)), function(err, data) {
                if (err) {
...
function updateHead(repopath, sha1, callback) {
    fs.readFile(ph.join(repopath,'HEAD'), function (err, data) {
        ...
        datastr = datastr.substr(0, datastr.length - 1);
        if (datastr.substr(0,5) === refs) {
            fs.writeFile(ph.join(repopath, datastr.substr(5, datastr.length - 5)), sha1, callback);
        } else {
            fs.writeFile(ph.join(repopath, 'HEAD'), sha1, callback);
        }
    });
}
