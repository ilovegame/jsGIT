...
var readObjectAndType = function(sha1, repoPath, callback)
{
    fs.readFile(getFilePath(sha1, repoPath), function (err, data) {
	    ...
	    //console.log(buffer.toString('utf-8', 0, buffer.length));
            ...
            //console.log("null na pozycji " + i);
            var type = buffer.slice(0, i);
	    var temp = type.toString('utf-8', 0, type.length);
	    type = temp.split(' ')[0];
...
//line format: author <mail> timeZone timestamp
{
	...
	var timestamp = words[words.length - 2];
	var timeZone = words[words.length - 1];
...
OperationDetails.prototype.toString = function() {
	return this.author + ' <' + this.authorMail + '> ' + this.timestamp + ' ' + this.timeZone;
};
...
exports.readObjectAndType = readObjectAndType;
