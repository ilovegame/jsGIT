    ...
    fs.readFile(this.indexPath, function (err, fileIndexContent) {
        ...
        while (index < stop) {
            if (indexFile.objectsNumber == numberOfIndexEntries) {
                Index.prototype.ignoreExtension.call(indexFile, fileIndexContent, index, function(err, extensionSize) {
                    if (err) {
                        callback(err);
                        return;
                    }
...
Index.prototype.addObject = function (path, callback) {
    var indexFile = this;
    console.log('0' + path)
    console.log('1' + Object.keys(indexFile.objects));
    this.hasObject(path, function (has) {
        if (!has) {
            ++indexFile.objectsNumber;
        }
        Index.prototype.createObjectInfo.call(indexFile, path, function (err, objectInfo) {
            console.log('2' + Object.keys(indexFile.objects));
            if (err) {
                callback(err);
                return;
            }
            Index.prototype.getRelativePath.call(indexFile, path, function(relativePath) {
                console.log('3' + Object.keys(indexFile.objects));
                indexFile.objects[relativePath] = objectInfo;
                console.log('4' + Object.keys(indexFile.objects));
                Index.prototype.createIndexFile.call(indexFile, function (err) {
                    console.log('5' + Object.keys(indexFile.objects));
                    if (err) {
                        callback(err);
                        return;
                    }
    ...
    Index.prototype.createHeader.call(indexFile, function (header) {
        Index.prototype.createDatas.call(indexFile, function (datas) {
            ...
            indexFileContent.length - 20), function (SHA_1) {
                ...
                20, 'hex');
                console.log('WOLO ' + Object.keys(indexFile.objects));
...
Index.prototype.getRelativePath = function (path, callback) {
    console.log('dziwne rzeczy ' + path.slice(ph.normalize(ph.join(this.repoPath,'..')).length + 1));
    callback(path.slice(ph.normalize(ph.join(this.repoPath,'..')).length+1));
    return;
}
