...
var uM = require('utils.js');
var commitM = require('commit');
var treeM = require('tree');
/**
...
function Index (repoPath, callback) {
    ...
    this.indexPath = ph.join(repoPath,'index');
...
Index.prototype.parse = function (callback) {
    ...
    fs.readFile(this.indexPath, function (err, fileIndexContent) {
        ...
        var index = 0;
        var signature = fileIndexContent.toString('utf8', index, index + 4);
        index += 4;
        ...
        while (index < stop) {
            if (indexFile.objectsNumber == numberOfIndexEntries) {           
                if (fileIndexContent.toString('utf8', index, index + 4) != 'TREE' &&
                    fileIndexContent.toString('utf8', index, index + 4) != 'REUC') {
                    callback('Error during parsing index file', '');
                    return;
                }                   
                var extensionSize = fileIndexContent.readInt32BE(index);
                index += extensionSize + 8;
            } else {
                ...
                index += 4;
                objectInfo['SHA_1'] = fileIndexContent.toString('hex', index,
                    index + 20);
                index += 20;
                ...
                var path = fileIndexContent.toString('utf8', index,
                    index + nullEndIndex);
...
Index.prototype.hasObject = function (path, callback) {
    var indexFile = this;
    Index.prototype.getRelativePath.call(indexFile, path, function(relativePath){
...
Index.prototype.calculateSHA_1 = function(data, callback) {
    ...
    sha1Creator.update(data);
    callback(sha1Creator.digest('hex'));
    return;
}
...
Index.prototype.createObjectInfo = function (path, callback) {
    ...
    fs.stat(path, function (err, stats) {
        ...
        fs.readFile(path, function(err, fileAddedContent) {
            ...
            blobUtils.fileToBlob(fileAddedContent, function(err, blob) {
                if (err) {
                    callback(err);
                    return;
                }
                Index.prototype.calculateSHA_1.call(indexFile, blob, function (SHA_1) {
                    objectInfo['SHA_1'] = SHA_1;
                    objectInfo['flag'] = pathBuffer.length;
                    Index.prototype.getRelativePath.call(indexFile, path.toString('utf8'), function(relativePath) {
...
Index.prototype.objectToIndexFormat = function (object, callback) {
    var objectInfoSize = 62;
    var pathBuffer = new Buffer(object['path'], 'utf8');
    var remainNulls = 8 - ((pathBuffer.length - 2) % 8);
    ...
    data.write(object['SHA_1'], index, object['SHA_1'].length, 'hex');
    index += 20;
    ...
    Index.prototype.getNulls.call(this, remainNulls, function(nulls) {
 ...
 * @param callback callback(err)
 */
Index.prototype.addObject = function (path, callback,version) {
    var indexFile = this;
    this.hasObject(path, function (has) {
        ...
        if (version) {
            Index.prototype.createObjectInfo2.call(indexFile, path, function (err, objectInfo) {
                if (err) {
                    callback(err);
                    return;
                }
                Index.prototype.getRelativePath.call(indexFile, path, function(relativePath) {
                    ...
                    Index.prototype.createIndexFile.call(indexFile, function (err) {
                        if (err) {
                            callback(err);
                            return;
                        }
            ...
            Index.prototype.createObjectInfo.call(indexFile, path, function (err, objectInfo) {
                if (err) {
                    callback(err);
                    return;
                }
...
Index.prototype.addObjectFromHEAD = function(path, callback) {
    var indexFile = this;
    Index.prototype.getRelativePath.call(indexFile, path, function(relativePath) {
        guM.getHeadSha1(indexFile.repoPath, function(err, headSHA_1) {
            if (err) {
                callback(err);
                return;
            }
            if (!headSHA_1) {
                callback('Error: No commit yet');
                return;
            }
            commitM.readFileToCommit(headSHA_1, indexFile.repoPath, function(err, headCommit) {
                if (err) {
                    callback(err);
                    return;
                }
                var getFileSHA_1 = function(tree, pathParts, index, callback) {
                    if (pathParts.length - 1 == index) {
                        if (!tree['entries'][pathParts[pathParts.length - 1]]) {
                            callback('Error: file ' + pathParts[pathParts.length - 1] + " not in commit");
                            return;
                        }
                        callback('', tree['entries'][pathParts[pathParts.length - 1]]['sha1']);
                        return;
                    }
                    treeM.readFileToTree(tree['entries'][pathParts[index]]['sha1'], indexFile.repoPath, function(err, tree) {
                        if (err) {
                            callback(err, '');
                            return;
                        }
                        getFileSHA_1(tree, pathParts, index + 1, function(err, fileSHA_1) {
                            if (err) {
                                callback(err, '');
                                return;
                            }
                ...
                treeM.readFileToTree(headCommit['objectId'], indexFile.repoPath, function(err, tree) {
                    if (err) {
                        callback(err);
                        return;
                    }
                    getFileSHA_1(tree, relativePath.split('/'), 0, function(err, fileSHA_1) {
                        if (err) {
                            callback(err);
                            return;
                        }
                        ...
                        fileInfo['ctime'] = 0;
                        fileInfo['ctime_fraction'] = 0;
                        fileInfo['mtime'] = 0;
                        fileInfo['mtime_fraction'] = 0;
                        fileInfo['dev'] = 0;
                        fileInfo['ino'] = 0;
                        fileInfo['mode'] = 0100644;
                        fileInfo['uid'] = 0;
                        fileInfo['gid'] = 0;
                        fileInfo['size'] = 0;
                        fileInfo['SHA_1'] = fileSHA_1;
                        fileInfo['flag'] = 0;
                        fileInfo['path'] = relativePath;

                        if (!(relativePath in indexFile.objects)) {
                            ++indexFile.objectsNumber;
                        }
                        indexFile.objects[relativePath] = fileInfo;
                        Index.prototype.createIndexFile.call(indexFile, function (err) {
                            if (err) {
                                callback(err);
                                return;
...
Index.prototype.removeObject = function (path, callback) {
    var indexFile = this;
    Index.prototype.hasObject.call(indexFile, path, function (has) {
        if (has) {
            Index.prototype.getRelativePath.call(indexFile, path, function(relativePath) {
...
/**
 * Removes all files from index file
 * @param callback callback(err)
 */
Index.prototype.removeAllObjects = function(callback) {
    var indexFile = this;
    var keys = new Array();
    for (var key in indexFile.objects) {
        keys.push(key);
    }
    
    var removeObjectsRecursively = function(index, callback) {
        if (keys.length == index) {
            callback('');
            return;
        }
        var path_prefix = indexFile.repoPath.slice(0, indexFile.repoPath.length - 4);
        Index.prototype.removeObject.call(indexFile, path_prefix + keys[index], function(err) {
            if (err) {
                callback(err);
                return;
            }
            removeObjectsRecursively(index + 1, function(err) {
                if (err) {
                    callback(err);
                    return;
                }
    ...
    removeObjectsRecursively(0, function(err) {
        if (err) {
            callback(err);
            return;
        }
...
Index.prototype.createDatas = function (callback) {
    ...
    if (this.objectsNumber > 0) {
        for (var object in this.objects) {
            Index.prototype.objectToIndexFormat.call(indexFile, this.objects[object], function (data) {
                datas.push(data);
                ++objectsAdded;
...
Index.prototype.createIndexFile = function (callback) {
    var indexFile = this;
    var t = this.objects;
    Index.prototype.createHeader.call(indexFile, function (header) {
        Index.prototype.createDatas.call(indexFile, function (datas) {
            var datasSize = 0;
            ...
            Index.prototype.calculateSHA_1.call(indexFile, indexFileContent.toString('binary', 0,
...
Index.prototype.getRelativePath = function (path, callback) {
    var normalizedPath = path.slice(ph.normalize(ph.join(this.repoPath,'..')).length+1);
    var index = 0;
    var newPath = '';
