...
Index.prototype.hasObject = function (path, callback) {
    var indexFile = this;
    callback(path in indexFile.objects);
    return;
}
 ...
 */
Index.prototype.calculateBlobSHA_1 = function(data, callback) {
    ...
    sha1Creator.update(store.toString('binary'));
    
    callback(sha1Creator.digest().toString('binary'));
    return;
}
...
Index.prototype.createObjectInfo = function (path, callback) {
    ...
    var path2  = ph.join(ph.normalize(this.repoPath + '..'), path);
    var indexFile = this;
    fs.stat(path2, function (err, stats) {
        ...
        fs.readFile(path2, function(err, fileAddedContent) {
            if (err) {
                callback(err, '');
                return;
            }
            indexFile.calculateBlobSHA_1(fileAddedContent, function (SHA_1) {
...
Index.prototype.getObject = function(path, callback) {
    ...
    this.hasObject(path, function(has) {
        ...
        callback('', indexFile.objects[path]);
    });
}
