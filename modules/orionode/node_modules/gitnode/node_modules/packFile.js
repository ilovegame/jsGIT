...
function getBaseObjectOffset (buffer, objectOffset)
{
	var index = 0;
...
function parseDeltaData (buffer, baseBuffer, callback)
{
...
Pack.prototype.handleDelta1Object = function(objectOffset, objectBuffer, callback)
{
	...
	var res = getBaseObjectOffset(objectBuffer, objectOffset);
	index = res[0];
	...
	this.readObject(offset, length, function (err, baseBuffer)
	{
		...
		zlib.inflate(objectBuffer.slice(index, objectBuffer.length), function(err, buffer) {
			...
			parseDeltaData(buffer, baseBuffer, callback);
		});
	});
...
{
        var header = readPackedObjectHeader(buffer);
        // res = [index, length, type]
        var index = header[0];
        var length = header[1];
        var type = header[2];
...
Pack.prototype.parseFooter = function (fd, callback)
{
	...
	fs.stat(pack.path, function (err, stats) {
		...
		var length = 20;
		pack.readFromFile(true, true, fd, pack.size-20, length, function (buffer, fd) {
...
// for unpacking without index:
exports.readPackedObjectHeader = readPackedObjectHeader;
exports.getBaseObjectOffset = getBaseObjectOffset;
exports.parseDeltaData = parseDeltaData;