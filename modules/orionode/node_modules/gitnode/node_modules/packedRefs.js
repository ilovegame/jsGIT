 ...
 */
PackedRefs.prototype.getNameFromPath = function(path) {
...
PackedRefs.prototype.getTagsNames = function(callback) {
	...
	for (var path in this.tags) {
		...
		tagInfo['name'] = this.getPathToDisplay(path);
		tagInfo['tag_SHA_1'] = this.tags[path]['SHA_1'];
		tagInfo['commit_SHA_1'] = this.tags[path]['objectPointedSHA_1'];
		data.push(tagInfo);
	}
...
PackedRefs.prototype.getBranchNameUsingSHA_1 = function(SHA_1, callback) {
	for (var path in this.branches) {
		if (this.branches[path] == SHA_1) {
			callback(this.getNameFromPath(path));
			return;
		}
	}
...
/**
 * Gets all branches which come from selected remote
 * @param remoteName name of the remote
 * @param callback callback(branchesNames)
 */
PackedRefs.prototype.getRemoteBranchesName = function(remoteName, callback) {
	var getRemoteNameFromBranchName = function(branchName) {
		var words = branchName.split('/');
		var wordIndex = 0;
		while (wordIndex < words.length && words[wordIndex] != 'remotes') {
			++wordIndex
		}
	...
	for (var branchPath in this.branches) {
		if (remoteName == getRemoteNameFromBranchName(branchPath)) {
			remoteBranches.push(packedRefsObject.getNameFromPath(branchPath));
		}
	}
