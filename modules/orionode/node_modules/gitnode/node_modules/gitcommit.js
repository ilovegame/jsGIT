 ...
 * Example:
 * 
   gcM.gitCommit('/home/marcinek/gitrepotest/.git/', 'normal', commitInfo, function(err, result) {
...
var commitWorkTreeSha1 = function(repopath, option, filepath, callback) {
    try {
        gsM.gitStatus(repopath, function(err, statusRes, treeDir, graph, index) {
            ...
            var treeSha1 = modifyGraph(graph, index, statusRes, option, filepath);
            if (treeSha1 === undefined) { 
                callback(err);
                return;
            }
            tM.graphToRepo(repopath, graph, function(err) {
                if(err) throw err;
                callback(undefined, treeSha1);
            });
        });
    } catch (err) {
        callback(err);  
...
var getCommitInfo = function(commitInfo) {
    ...
    var authorInfo = new uM.OperationDetails(commitInfo.author,
                                             commitInfo.authorMail, timestamp, timeZone); 
    var committerInfo = new uM.OperationDetails(commitInfo.committer,
                                                commitInfo.committerMail, timestamp, timeZone);
    return {'tz': timezone, 'ts':timestamp, 
        'aI' : authorInfo, 'cI' : committerInfo, 'desc' : commitInfo.description}
}


exports.gitCommitIndex = function(repopath, commitInfo, callback) {
    var cI = getCommifInfo(commitInfo);
    var res = {};
    try {
        commitWorkTreeSha1(repopath, function(err, treeSha1) {
            if (err) throw err; 
            if (!treeSha1) {
                callback(null, res);
                return;  
            ...
            guM.getHeadSha1(repopath, function(err, sha1) {
                if(err) throw err;
                var commitObject = new cM.Commit(treeSha1, [sha1],
                    cI.desc, cI.aI, cI.cI);
...
exports.gitAmendIndex = function(repopath, commitInfo, callback) {
    
}



exports.gitCommit = function(repopath, option, commitInfo, callback) {
    ...
    try {
        gsM.gitStatus(repopath, function(err, statusRes, treeDir, graph, index) {
            ...
            var function1 = function(err, headSha1) {
                ...
                tM.graphToRepo(repopath, graph, function(err, sh) {
                    if(err) throw err;
...
exports.gitCommitAmend = function(repopath, option, commitInfo, callback) {
    var parentId = undefined;
    var result = {};
    var date = new Date();
    var timeZoneN = -(date.getTimezoneOffset())/60;
    ...
    try {
        gsM.gitStatus(repopath, function(err, statusRes, treeDir, graph, index) {
            if (statusRes.changed.length === 0 && statusRes.added.length === 0 &&
                statusRes.removed.length === 0) {
                var headCommit = guM.getHeadCommit(repopath, function(
                var commitObject = new cM.Commit(treeSha1, parents,
                        commitInfo.description, authorInfo, committerInfo);
                var newHeadSha1 = commitObject.getSha1();
                commitObject.save(repopath, function(err) {
