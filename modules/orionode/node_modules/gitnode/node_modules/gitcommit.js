...
function removeGraph(path, graph, graphModified) {
    ...
    for (var i = graphEntries.length - 1; i >= 0; --i) {
        if(previousEmpty)
 ...
 * @param callback(err, commitObject);  commitObject -> object from commit.js
 * 
 ...
 * Example:
 * 
   gcM.gitCommit('/home/xx/gitrepotest/.git/', commitInfo, function(err, commitObject) {
...
//commit amend , description is similar to gitCommitIndex
    ...
    var doFileWork = function(filepath, sha1_1, sha1_2) {
        ...
        if(sha1_1 && !sha1_2) {
            ...
            diffM.getDiffCommitCommit(repopath, commitheadsha1, commitsha1, function(err, diffs) {
                ...
                if (!diffs[filepath] || diffs[filepath].length === 1) {
                    okFiles.push(ph.join(ph.normalize(ph.join(repopath,'..')), filepath));
                    if (!--pending) doIndexWork();
                } else {
                    result = 'Conflicting';
